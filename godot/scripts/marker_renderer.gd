...
func _add_character_figure(root: Node3D, tint: Color, skin: String) -> void:
	...
	# Official character skin as Y-billboard portrait (readable at distance)
	var tex := ArtifactsAssets.character_texture(skin)
	if tex != null:
		var portrait := MeshInstance3D.new()
		var quad := QuadMesh.new()
		quad.size = Vector2(0.72, 0.96)
		portrait.mesh = quad
		portrait.material_override = ArtifactsAssets.billboard_material(tex, tint)
		portrait.position = Vector3(0.0, y0 + 1.55, 0.38)
