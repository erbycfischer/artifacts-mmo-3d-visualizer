...
const PlacePropsScript = preload("res://scripts/place_props.gd")
...
func _maybe_add_biome_clutter(tile: Dictionary) -> void:
	...
	if "water" in skin or "lake" in skin or "interior" in skin:
		return
	var density := clutter_density
	if "sand" in skin or "desert" in skin or "beach" in skin:
		density = clutter_density * 0.45
	elif "snow" in skin or "ice" in skin:
		density = clutter_density * 0.7
	...
	var seed_key := int(tile.get("x", 0)) * 31 + int(tile.get("y", 0)) * 17

	if "mountain" in skin or "mine" in skin or "underground" in skin:
		if not _try_add_kenney(root, _props().rock(seed_key)):
	...
	if "sand" in skin or "desert" in skin or "beach" in skin:
		if roll > 0.55:
			if not _try_add_kenney(root, _props().rock(seed_key)):
				_add_rock_cluster(root, 0.4 + roll * 0.25)
		elif not _try_add_kenney(root, _props().crop(seed_key)):
			_add_bush(root, 0.25 + roll * 0.2)
		_add_blob_shadow(root, 0.3)
		return

	if "snow" in skin or "ice" in skin or "pine" in skin:
		if not _try_add_kenney(root, _props().pine(seed_key)):
			_add_ambient_tree(root, 0.65 + roll * 0.35)
		_add_blob_shadow(root, 0.48)
		if roll < 0.25:
			if not _try_add_kenney(root, _props().rock(seed_key + 3)):
				_add_rock_cluster(root, 0.3)
		return

	var kind := _hash01(int(tile.get("x", 0)), int(tile.get("y", 0)) + 21)
	if kind > 0.58:
		var tree_node: Node3D = _props().tree(seed_key) as Node3D
		if not _try_add_kenney(root, tree_node):
			_add_ambient_tree(root, 0.55 + kind * 0.5)
		_add_blob_shadow(root, 0.5)
		if kind > 0.82:
			if not _try_add_kenney(root, _props().grass(seed_key + 3)):
				_add_grass_patch(root, tile)
	elif kind > 0.32:
		if not _try_add_kenney(root, _props().bush(seed_key)):
			_add_bush(root, 0.4 + kind * 0.3)
		_add_blob_shadow(root, 0.35)
		if kind < 0.42:
			if not _try_add_kenney(root, _props().grass(seed_key + 5)):
				_add_grass_patch(root, tile)
	else:
		if not _try_add_kenney(root, _props().grass(seed_key)):
			_add_grass_patch(root, tile)
		_add_blob_shadow(root, 0.28)
		if kind < 0.12:
			if not _try_add_kenney(root, _props().rock(seed_key + 7)):
				_add_rock_cluster(root, 0.35 + kind)


func _dress_settlement_paths(maps: Array, routes: Array) -> void:
	...
	for route in routes:
		...
		for i in range(pts.size() - 1):
			if not (pts[i] is Dictionary and pts[i + 1] is Dictionary):
				continue
			for cell in _bresenham_cells(pts[i], pts[i + 1]):
				path_cells[cell] = true
	# Also connect major POIs when routes are sparse.
	var pois: Array = []
	for tile in maps:
		if not (tile is Dictionary):
			continue
		if str(tile.get("layer", "overworld")) != "overworld":
			continue
		var ct := _content_type(tile)
		if ct in ["bank", "grand_exchange", "workshop", "npc"]:
			pois.append(tile)
	for i in range(pois.size()):
		for j in range(i + 1, pois.size()):
			var a: Dictionary = pois[i]
			var b: Dictionary = pois[j]
			var dist := absi(int(a.get("x", 0)) - int(b.get("x", 0))) + absi(int(a.get("y", 0)) - int(b.get("y", 0)))
			if dist > 8:
				continue
	...
	for key in path_cells.keys():
		...
		if parts.size() < 2:
			continue
		var fake := {"x": int(parts[0]), "y": int(parts[1]), "layer": "overworld"}
		var existing: Variant = _tile_data.get(_tile_key(fake), null)
		if existing is Dictionary:
			var skin := str((existing as Dictionary).get("skin", "")).to_lower()
			if "water" in skin or "lake" in skin:
				continue
		var stone: Node3D = _props().path_stone(idx) as Node3D
		if stone == null:
			idx += 1
			continue
		stone.position = grid_to_world(fake) + Vector3(0.0, 0.01, 0.0)
		stone.rotation_degrees.y = float(idx * 37 % 360)
		_props_root.add_child(stone)
		idx += 1


func _bresenham_cells(a: Dictionary, b: Dictionary) -> Array:
	var x0 := int(a.get("x", 0))
	var y0 := int(a.get("y", 0))
	var x1 := int(b.get("x", 0))
	var y1 := int(b.get("y", 0))
	var cells: Array = []
	var dx := absi(x1 - x0)
	var dy := -absi(y1 - y0)
...
func _props():
	if _place_props == null:
		_place_props = PlacePropsScript.new()
	return _place_props
...
func _add_content_prop(tile: Dictionary, content_type: String, content_code: String) -> void:
	...
	if tex != null:
		...
		var is_creature := content_type == "monster" or content_type == "raid"
		if is_creature:
			# Official monster art as primary front billboard
			var size := 1.35 if content_type == "raid" else 1.1
			quad.size = Vector2(size, size)
			accent.position = Vector3(0.0, 1.15 if content_type != "raid" else 1.35, 0.55)
		elif content_type == "npc" or content_type == "tasks_master":
			quad.size = Vector2(0.85, 0.95)
			accent.position = Vector3(0.0, 1.45, 0.45)
		else:
...
func _add_tree_prop(root: Node3D, code: String) -> void:
	var seed_key := code.hash()
	var kenney: Node3D = _props().tree(seed_key) as Node3D
	if code.to_lower().find("ash") >= 0 or code.to_lower().find("tree") >= 0:
		kenney = _props().tree(seed_key)
	if kenney != null:
		root.add_child(kenney)
		_register_wind(kenney)
		return
...
func _add_creature_prop(root: Node3D, content_type: String, code: String) -> void:
	var tint := Color(0.78, 0.26, 0.2) if content_type == "monster" else Color(0.55, 0.12, 0.1)
	var scale := 1.2 if content_type == "raid" else 1.0

	# Compact silhouette pedestal so the official sprite reads as the identity.
	var hips := MeshInstance3D.new()
	var hips_mesh := SphereMesh.new()
	hips_mesh.radius = 0.22 * scale
	hips_mesh.height = 0.32 * scale
	hips.mesh = hips_mesh
	hips.position.y = 0.28 * scale
	hips.scale = Vector3(1.15, 0.8, 0.95)
	hips.material_override = _cached_prop_mat("creature_hips", tint.darkened(0.15))
	root.add_child(hips)

	var body := MeshInstance3D.new()
	var capsule := CapsuleMesh.new()
	capsule.radius = 0.22 * scale
	capsule.height = 0.7 * scale
	body.mesh = capsule
	...
	head.material_override = _cached_prop_mat("creature_head", tint.lightened(0.1))
	root.add_child(head)

	var tex: Texture2D = ArtifactsAssets.content_texture(content_type, code)
	if tex != null:
		var sprite := MeshInstance3D.new()
		var quad := QuadMesh.new()
		var side := 1.15 * scale if content_type == "raid" else 0.95 * scale
		quad.size = Vector2(side, side)
		sprite.mesh = quad
		sprite.material_override = ArtifactsAssets.billboard_material(tex, tint)
		sprite.position = Vector3(0.0, 0.95 * scale, 0.0)
		...
		for side_sign in [-1.0, 1.0]:
			...
			var cone := CylinderMesh.new()
			cone.top_radius = 0.0
			cone.bottom_radius = 0.06 * scale
			cone.height = 0.28 * scale
			horn.mesh = cone
			horn.position = Vector3(side_sign * 0.14 * scale, 1.28 * scale, -0.02)
			horn.rotation_degrees = Vector3(-15.0, 0.0, side_sign * -28.0)
...
func _add_building_prop(root: Node3D, content_type: String) -> void:
	var seed_key := content_type.hash()
	var kenney: Node3D = null
	match content_type:
		"bank":
			kenney = _props().bank_building(seed_key) as Node3D
		"grand_exchange":
			kenney = _props().ge_building(seed_key) as Node3D
		"workshop":
			kenney = _props().workshop_building(seed_key) as Node3D
		_:
			kenney = null
	if kenney != null:
		root.add_child(kenney)
		_add_place_beacon(root, content_type)
		return
...
func _add_place_beacon(root: Node3D, content_type: String) -> void:
	## Colored ground ring so bank / GE / workshop read apart at distance.
	...
	var tint := _content_fallback(content_type)
	var mat := StandardMaterial3D.new()
	mat.albedo_color = Color(tint.r, tint.g, tint.b, 0.75)
	mat.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	mat.emission_enabled = true
	mat.emission = tint
	mat.emission_energy_multiplier = 0.85 if content_type == "grand_exchange" else 0.65
	...
	match content_type:
		...
		"workshop":
			label.text = "Workshop"
		_:
			label.text = content_type.capitalize()
	label.font_size = 20
	label.outline_size = 7
	label.position = Vector3(0.0, 2.35 if content_type != "workshop" else 1.85, 0.0)
	label.billboard = BaseMaterial3D.BILLBOARD_ENABLED
	root.add_child(label)


func _add_npc_prop(root: Node3D) -> void:
	var stall: Node3D = _props().npc_stall(42) as Node3D
	if stall != null:
...
func _add_event_prop(root: Node3D) -> void:
	var fire: Node3D = _props().campfire(11)
	if fire != null:
		root.add_child(fire)
	var shelter: Node3D = _props().tent(22)
	if shelter != null:
		shelter.position = Vector3(0.55, 0.0, -0.35)
...
func _skin_height(skin: String) -> float:
	...
	if "pine" in s:
		return 0.06
	if "sand" in s or "desert" in s or "beach" in s:
		return 0.02
	if "underground" in s:
		return -0.05
	if "interior" in s:
